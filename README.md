...
前端项目位于 `ai-cv-optimize-web/` 目录，基于 Vue 3 + TypeScript + Element Plus 构建。
...
| 登录页 | `/login` | 用户登录/注册 |
| 首页 | `/` | 系统介绍和功能导航 |
| 简历优化 | `/optimize` | 简历上传和优化配置 |
| 职位选择 | `/job-selection` | 智能推荐职位选择 |
| 优化结果 | `/result` | 展示诊断报告和优化结果 |
| 面试模拟 | `/interview` | AI 面试练习 |
| 知识库 | `/rag` | RAG 文档管理 |
...
前端开发服务器已配置代理，自动转发 `/api` 和 `/rag` 请求到后端 http://localhost:8080
